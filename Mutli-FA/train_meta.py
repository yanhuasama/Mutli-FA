...
def main(config):
    ...
    if config.get('load_prompt'):
        token_feat = torch.tensor(np.load(config.get('load_prompt'))).cuda()
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_loader, desc='train', leave=False):
            ...
            text_feat = torch.zeros(0).cuda()
            for i in range(len(labels)):
                text_feat = torch.cat((text_feat,token_feat[labels[i]].view(1,-1)),dim=0)

            # logits = model(x_shot, x_query).view(-1, n_train_way)
            logits = model(text_feat, x_shot, x_query).view(-1, n_train_way)
        ...
        for name, loader, name_l, name_a in [
            ...
            for data, _ in tqdm(loader, desc=name, leave=False):
                ...
                for i in range(len(labels)):
                    if name == 'tval':
                        text_feat = torch.cat((text_feat,token_feat[labels[i]+80].view(1,-1)),dim=0)#80/448
                    else:
                        text_feat = torch.cat((text_feat,token_feat[labels[i]+64].view(1,-1)),dim=0)#64/351
