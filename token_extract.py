    ...
    with open(json_file, 'r', encoding='utf-8') as file:
        ...
        for label, prompt in json_data.items():
            ...
            with torch.no_grad():
                ...
                if actionembed.size(0)>1:
                    actionembed = 0.9 * actionembed[:1] + 0.1 * torch.mean(actionembed[1:], dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='miniImagenet', help='image dataset')
    args = parser.parse_args()

    json_file = os.path.join('./json', args.dataset+'-concept_prompt.json')
    prompt_file = os.path.join('./prompt_feature', args.dataset+'-')
